 ...
 * engine-contract.md §8). Deliberately exposes concealed hand tiles and
 * undrawn wall tiles; callers (server) must gate access themselves. Generic
 * over any ruleset whose state shares the `{ wall, seats }` shape, same
 * assumption `assertContainerUniqueness` already makes.
 */
