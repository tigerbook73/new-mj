...
export const priority = (action: HangzhouClaimAction): number =>
...
export const chooseClaims = (
  state: HangzhouState,
): Array<{ seat: SeatId; action: HangzhouClaimAction }> => {
...
export const allResponded = (state: HangzhouState): boolean => {
  const pending = state.pendingClaims!;
...
export const actionEquals = (left: HangzhouAction, right: HangzhouAction): boolean =>
