...
} from "../src/evaluation/comparator.ts";
import {
  executeCalibrationTasks,
  executeCalibrationTasksInWorkers,
} from "../src/evaluation/executor.ts";
import {
  runCalibrationEvaluationsWithExecutor,
...
describe("Junk evaluation runner integration", () => {
  it("runs a real fixture and matches its production baseline", () => {
    ...
    expect(
      compareCalibrationBaseline(baseline as CalibrationBaseline, report.evaluations[0]!).status,
    ).toBe("matched");
  ...
  it("produces equivalent results through sequential and worker executors", async () => {
    ...
    const worker = await runCalibrationEvaluationsWithExecutor(
      ...
      [normalized],
      (tasks) =>
        executeCalibrationTasksInWorkers(tasks, {
          workerCount: 1,
          workerUrl: new URL("../src/evaluation/worker.ts", import.meta.url),
          moduleUrl: new URL(
            "../src/junk/evaluation/production-evaluator-task.ts",
            import.meta.url,
          ),
