...
type JunkStrategyConfig = Readonly<{ sevenPairs?: boolean; robKong?: boolean }>;
...
export const scoreHandShapeAfterDiscard = (
  ...
  const shanten = junkShanten(hand, { sevenPairs: config.sevenPairs ?? true });
  // 进张枚举会再求 34 次向听数；离听牌尚远时，先以向听数本身做筛选即可，
  // 避免自动对局在每一次出牌都做无收益的二层穷举。
  const improvements =
    shanten <= 1 ? ukeire(hand, { sevenPairs: config.sevenPairs ?? true }).length : 0;
