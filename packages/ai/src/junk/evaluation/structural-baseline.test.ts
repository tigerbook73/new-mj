...
import manifestData from "./fixtures/structural-baseline-v5.json" with { type: "json" };
import {
...
describe("structural-baseline-v5 manifest", () => {
  ...
  it.each(manifestData.behavior)("locks $scenarioId", ({ scenarioId, expectedAction }) => {
    ...
    const baseline = recommendStructuralBaselineV5Action(input.view, input.legalActions);
    ...
    const alternatives: JunkAction[] =
      type === "hu"
        ...
        : type === "zimo"
          ...
          : [action];
    const baseline = recommendStructuralBaselineV5Action(flowView(phase), alternatives);
