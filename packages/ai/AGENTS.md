...
- 调参默认权重是数据、不是代码：`src/junk/default-weights.json`（`import ... with { type: "json" }`），`DEFAULT_JUNK_WEIGHTS` 从这个文件加载并 `Object.freeze`，不再手写字面量。`evaluation/commands/tune-cli.ts --write` 是唯一允许写这个文件的入口，且只在人工显式传了这个 flag、held-out 评估显示候选权重没有变差时才写——不自动、不默认。
