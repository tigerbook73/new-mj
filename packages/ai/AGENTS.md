...
- Junk 生产依赖保持 `weights + analysis → hand-quality → two-ply → action-scoring → strategy facade` 单向流动；`strategy.ts` 是兼容加载根，只保留最终动作选择并重导既有诊断 API。生产路径不得 import `evaluation/`。
