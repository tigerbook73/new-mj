...
- 自对弈/调参驱动器（如 `src/junk/arena.ts`、`src/junk/tune.ts`）本身要有对应的 slow-tag 测试、走标准 `verify`/`verify:full` 链，证明管线跑得通；但**真正跑一次有意义的调参/大规模自对弈是人工触发的**，不进 `test`/`verify` 依赖链，只通过 package 的统一 `evaluate` 命令族（如 `evaluate weights tune`）手动调用。工具实现保持在 `src/` 内以接受 `tsconfig.json`/ESLint 覆盖，但不从包的公共 `index.ts` barrel 导出。
