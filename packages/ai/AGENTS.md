...
- Junk 默认生产使用普通标准型 + 七对 + 门清 claim 阈值 + 碰碰胡弃牌 tiebreak + 清一色/混一色弃牌 tiebreak 的无权重结构 facade（七对接入范围与惩罚设计见 `docs/architecture/shanten.md`"七对结构路线"节，门清阈值见同文件"门清 claim 阈值"节，碰碰胡见同文件"碰碰胡结构路线"节，清一色/混一色见同文件"清一色/混一色弃牌方向"节）；`strategy.ts` 是稳定加载根，只保留最终动作选择并重导结构诊断 API。生产路径不得 import `evaluation/`。
