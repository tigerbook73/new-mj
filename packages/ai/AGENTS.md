...
- **跨版本/跨配置批量对比**（`src/junk/policy-loader.ts` 是三个工具共用的加载器）：
  ...
  - 两个都没提交、想互相对比的实验版本（还没到"哪个是历史哪个是新版"的地步，`ref` 用不上）：`pnpm capture:junk-policy <label>` 把当前 policy 依赖复制到 `packages/ai/.compare-scratch/<label>/junk/`（gitignored，刻意放在 `src`/`test` 外，不会被 `tsconfig.json`/`eslint src test` 扫到），之后继续在 `src/junk/` 原地改，用 `--baseline-module .compare-scratch/<label>/junk/strategy.ts` 对比——不需要复制完再恢复的手工操作。
