...
**阶段 1.5 尚未验收**：Stage A 已落地 `playing ⇄ awaiting-claims` 的缺门出牌、碰/胡声明、头跳/一炮多响、多赢家离场、三家胡/牌墙耗尽出口，并已接入 `engine.ts` 注册表；杠分、抢杠胡和呼叫转移已落地，终局查花猪/退税/查大叫和 fuzz 仍未完成。
...
**Stage B 进行中**：暗杠、补杠、直杠已实现基础副露、在场付款、杠分账本和补摸；补杠抢杠胡窗口已接入，抢杠胡不产生杠分付款；杠上炮呼叫转移已接入并有付款账本 fixture。退税/终局检查和完整 fuzz 尚未完成。

**下一步第一个动作**：实现流局查花猪、退税与查大叫的结算顺序，并为每个付款人补精确账本 fixture。
