...
## 阶段 2：NestJS Server 架构设计（规格阶段）

> **状态**：规格完成，待实施  
> **详细文档**：[docs/phase-2-server.md](./phase-2-server.md)  
> **工期估计**：7-10 天单人；4-5 天并行  

### 完成内容

- ✅ 房间模型设计：[docs/rooms.md](./rooms.md)（4-round 会话，支持 best-of-3 扩展）
- ✅ Protocol 类型定义：`packages/protocol/src/schemas.ts`（RoomInfo、SessionFormat 等）
- ✅ NestJS 架构规格：[docs/phase-2-server.md](./phase-2-server.md)（5 阶段分解）

### 待做任务

**实施路线**（按阶段）：
- [ ] **阶段 1**（1-2 天）：NestJS 脚手架 + GameService 包装
- [ ] **阶段 2**（0.5 天，可与 1 并行）：CLAUDE.md + AGENTS.md（package 规范）
- [ ] **阶段 3**（1-2 天）：RoomService 编排 + EventBus + 核心流程
- [ ] **阶段 4**（2-3 天）：Socket.IO Gateway + 可见性过滤 + 错误处理
- [ ] **阶段 5**（1-2 天）：E2E 集成测试 + 单元测试

**验收标准**：
- 4 玩家创建房间 → 加入 → 就绪 → 开始 ✅
- Core engine 驱动 4 局完整游戏 ✅
- 分数累加 + 最终排名 ✅
- `pnpm verify` 全绿 ✅
...
- **N=4（半庄）**：固定 4 局，简单清晰
- **庄家轮转**：MVP 简单顺时针；future 支持赢家继续当庄
- **可见性过滤**：用 core 提供的 `eventsVisibleTo()`，server 不理解规则
- **事件流**：ack/event 分离（ack 回执，event 广播）
