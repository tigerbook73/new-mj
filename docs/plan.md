...
本轮 engine-api 类型边界收窄：公共 `PlayerViewBase` 只保留跨玩法共同的 `handCount` seat 信息；junk 在自己的 `JunkPlayerView` 中声明 TileId 形式的副露/牌河，bloodbattle 在 `BloodbattlePlayerView` 中声明对外安全的 TileKind 形式，避免公共骨架混用不同玩法的数据形状。

**下一步第一个动作**：按 proposal 的 Stage A/B/C 清单逐项完成阶段 1.5 验收审计，核对规则、事件、守恒、fuzz 和文档状态，确认是否可以标记阶段完成并归档 proposal。
