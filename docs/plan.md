...
**阶段 1.5 尚未验收**：`playing ⇄ awaiting-claims` 的出牌/吃碰杠/胡牌/多赢家/杠分实时结算/终局查花猪查大叫都还没做，也没接入 `engine.ts` 的 ruleset 注册表、没有 fuzz——这部分体量上相当于阶段 1 当年的"Junk 完整流程"步骤，是下一轮的工作。
...
**下一步第一个动作**：针对上面剩余的开放问题（#1/#2/#3/#4/#6），动手写 `playing` 阶段的出牌/声明窗口/多赢家结算，实现时顺带把 `BloodbattleAction`/`BloodbattleApplyResult` 补齐并注册进 `engine.ts`。
