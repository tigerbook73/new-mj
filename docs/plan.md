...
**阶段 1.5 尚未验收**：Stage A 已落地 `playing ⇄ awaiting-claims` 的缺门出牌、碰/胡声明、头跳/一炮多响、多赢家离场、三家胡/牌墙耗尽出口，并已接入 `engine.ts` 注册表；杠分、抢杠胡、呼叫转移、终局查花猪/退税/查大叫和 fuzz 仍未完成。
...
**Stage B 进行中**：暗杠已实现基础副露、在场付款、杠分账本和补摸；`buGang`/`minGang` 已接入动作与声明窗口。抢杠胡、呼叫转移、退税/终局检查和完整三种杠型 fixture 尚未完成。

**下一步第一个动作**：为 `buGang`/`minGang` 补精确付款 fixture，然后实现补杠抢杠胡窗口和呼叫转移。
