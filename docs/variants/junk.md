...
`seats[i].winSnapshot?: { hand: TileKind[]; winTile: TileKind; groups: TileKind[][] }`——**公开**，仅胡牌那一刻起该座位才有此字段（`state.wins[seat]` 落地时写入，直到整局结束不再清除），仿血战到底 `WinSnapshot`/`PublicWinSnapshot` 的公开揭示模式：`hand`/`winTile` 在私有状态（`TileId`）与 `PlayerView`（转换成 `TileKind`）之间做边界转换，`groups` 是实际用来判定胡牌的拆分（4 面子+1 将，或七对型的 7 组），本来就是 kind 级别不需要转换。`groups` 的家族优先级（先试基本型再试七对，`own.melds.length===0` 时）与 `isWin` 逐字复刻，保证与实际判定一致，不存在多解歧义。`hand`/`groups` 的展示排序规范见 `contracts/engine-contract.md` §5。
...
- 首局定庄（`computeInitialJunkDealer`）消费的是与洗牌同一条 PRNG 流（同一 seed 的第一个 `nextInt`），公开的庄家座位理论上泄漏约 2 bit 洗牌信息（把暴力枚举 seed 的空间缩小 4 倍）。非商用项目不做去相关处理，记录备查。
