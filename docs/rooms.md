...
const ranking = scores
  ...
  .sort((a, b) => 
    b.wins - a.wins || b.score - a.score // 先按赢局数，再按分数
  );
```
...
```
rooms/
  ├─ repository.ts      // Room CRUD、state 转移
  ├─ session.ts         // 单个房间的完整逻辑
  └─ ...
```

- `RoomService.create(rulesetId, config)` → roomId
- `RoomService.join(roomId, userId)` → Player 加入某座位
- `RoomService.ready(roomId, userId, ready)` → 更新 ready 状态
- `RoomService.start(roomId)` → 触发 waiting → round-1，生成 gameSnapshot（由 core 的 `createGame` 返回）
- `RoomService.nextRound(roomId)` → 内部逻辑，自动或手动触发
