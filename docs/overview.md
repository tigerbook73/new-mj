...
阶段 0–4（规则定稿、core 引擎、server、web 竖切、垃圾胡打磨到完整可玩：AI 对战 + UI 重做 + Replay）已完成；阶段 5（持久化：事件日志/战绩落 PG + 真正的 Supabase OAuth）代码已完成，等用户提供真实 Supabase 项目/OAuth Client secret 才能端到端验证。文档结构本身是在阶段 2 完成、阶段 3 开工前那个节点重构的（当时边界相对最"干净"，趁早把公共/专用的分界定下来），见 `doc-map.md` 的 v2 说明。
