...
decisions.md                    决策记录（轻量索引，记录级；维护规则见 §2.2）
```
...
| 产品级·活文档         | `process/workflow.md`（流程细则）                                                                                            | AI / 协作者               | 按需查阅，不要求每次开工通读；放 DoD/依赖维护/测试工具/Git 等展开的操作步骤；与 `AGENTS.md` 的分工见 §2.4                      |
| 记录级                | `decisions.md`                                                                                                               | 人 + AI                   | 轻量索引，多数条目上限 1-2 句；维护规则（豁免/精简判断）见 §2.2                                                                |
...
### 2.2 decisions.md 的精简规则（2026-07 修订：个人项目，轻量索引优先）

`decisions.md` 不再是"完整推理必须永久保留"的档案，而是一份轻量索引——大多数条目上限 1-2 句。默认允许精简/改写正文；不严格 append-only，但编号默认保留（不复用、不删空占位，避免误导"决策不存在过"），除非条目已彻底无价值。

**豁免（保留完整推理，不受 1-2 句上限约束）**：仅限确立了可复用判定标准或原则、未来同类问题需要参照的条目（如 D8 的配置边界标准、D19/D20 的"是否有规则语义"判定法）——这类条目的价值是给未来做参照，不是描述过去，精简了等于把判定标准也删掉。

**其余条目的两条路径**：

1. **指针替换**：结论 + 推理已经被 `AGENTS.md`/`architecture/*.md`/`contracts/*.md` 等活文档承接（逐字或近似逐字），原文替换为一行"已并入 `<path>`，原文见 git 历史"；同一 commit 内把引用该编号的其他文档改为直接指向新落脚文档。
2. **压缩为存根**：没有被别处承接，但也不属于豁免类——压成"结论 + 一句话理由（如适用：已被 `<Dn>` 取代）"，1-2 句为限；不写"已并入"（没有活文档承接）。一次性技术栈/库选型、已被后续决策取代、纯实现细节（env 文件组织之类）都归这类，不因为"没人承接"就必须保留完整叙事——个人项目里，完整推理留在 git 历史已经够用。

判断某条目是否属于豁免类的经验法则：**删掉这条的完整推理，未来做类似决策的人会不会因此重复踩坑或误判**——会，才豁免；只是"当时选了 A 没选 B"的一次性记录，不会。

### 2.3 过程文档的详略原则（2026-07-27 补充）
...
- 判断是否需要保留完整叙事：看它是否确立了未来同类工作要参照的设计取舍——是，按 §5 分流进 `decisions.md`/`architecture/*.md` 等活文档（该活文档里可以详细，过程文档里的条目只留指针）；不是，直接压缩，完整过程留 git 历史已经够用。

### 2.4 `AGENTS.md` 与 `process/workflow.md` 的分工
