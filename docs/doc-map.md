...
允许精简某条目的正文，不删除编号本身（编号不复用，避免误导"决策不存在过"），有两条互相独立的路径：

**路径一·指针替换**：原文迁移进活文档，本文件只留指针。仅当**同时满足**才适用：

1. 该决策的完整推理（不只是结论）已经逐字或近似逐字固化进某篇活文档，活文档不再需要指回本文件补充"为什么"；
2. 全仓库没有其他文档单靠条目编号（如 `D3`、评审点 `H`）来引用这条决策的独有推理。

操作方式：原文替换为一行"已并入 `<path>`，原文见 git 历史"的指针；同一 commit 内把所有引用该编号的文档改为直接指向新的落脚文档，不再经过 `decisions.md` 转一手。

**路径二·压缩为存根**（2026-07 新增）：原文没有被别处承接，但也判断不再需要保留完整推理。判断标准是**这条决策是否还有指导后续同类决策的价值**——若删掉这条的完整推理，未来做类似决策的人会不会因此重复踩坑或误判；不会，才能精简。典型可精简对象：一次性技术栈/库选型（结果已经固化进代码、不会被重新权衡）、已被后续决策明确取代且旧模型对现存代码不再有解释力的条目。**不适用于**：确立了可复用判定标准或原则、未来同类问题要参照的条目（如 D8 的配置边界标准、D19 的"是否有规则语义"判定法）——这类即使"已经落地"也保留完整推理，因为它们的价值正是给未来做参照，不是描述过去。操作方式：原文压缩为一行"结论 + 一句话理由 +（如适用）已被 `<Dn>` 取代"，不写"已并入"（没有活文档承接这段推理，只是判断它不再需要保留）；不涉及外部文档更新——引用该编号的指针依然指向同一条目，只是内容变短。
