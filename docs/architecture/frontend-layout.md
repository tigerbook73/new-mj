...
- **动画运行时边界**：动画基础设施集中在 `features/mahjong/animation/`：纯 reconciliation/几何在该目录的 model 文件，浏览器 DOM 读取与模块级瞬时状态由 coordinator 统一管理，portal/FLIP React 壳在其 `components/`。`TableView` 只在快照落库前调用 coordinator、在挂载和换局时 reset；`useTablePresentation` 只从权威 `PlayerView` 派生业务展示数据，不读取动画 singleton。组件通过只读 animation metadata 消费 token、源 rect 与 slot resolution。这个边界不改变 snapshot 权威性，也不让动画状态进入 session store。
- **四方手牌重排**：`HandReflowShell` 在 `TileMotion` 外层独占重排 transform；它以同一 visual token 的前后 `getBoundingClientRect()` 计算屏幕位移，再按座位 Zone 的逆旋转映射回局部坐标。四方都走这条路径：bottom/top 是 0/180 度的退化情况，left/right 才需要四分之一旋转换算。明牌 token 使用 TileId；普通对手暗牌使用只存于 Web 动画模块的 `back-slot` token。暗牌路径绝不向 ledger 输入 omniscient TileId，且不得在任何 DOM 属性（含 `data-hand-token`）写入 TileId。首帧、重连、重复快照、复杂多牌变化及 `prefers-reduced-motion` 只同步最终 token 布局、不补播。
- **跨区域飞行**：摸牌/出牌/认领的适用动画统一走独立临时 ghost（`components/useFlightGhost.ts`：测量一次 rect → portal 到 `document.body` → 播完自毁），从不让真实业务节点（牌河墓碑、副露牌、待摸槽位）承担跨容器动画或被这层触碰。不用 `layoutId` 共享布局——墓碑永不卸载，隐式共享布局会把它当成正在退场，和自身的 `dimmed` 目标打架（具体机制见 `ClaimFlipGhost.tsx`）。出牌 ghost 的源 rect 由快照提交前的视觉 token 捕获：明牌精确对应实际 TileId，暗牌则稳定选择一个可见牌背 token；后者仅是表现身份，不表示真实暗牌。暗牌 token 的重排壳与飞行源必须分开：前者可以是满高的 `HandReflowShell`，后者必须是内部实际牌尺寸 anchor 的 rect。portal 外盒直接使用该屏幕 rect 平移/缩放到牌河；left/right 仅在内部牌面保持固定的座位朝向并交换其未旋转尺寸，绝不在飞行途中旋转或反推局部坐标。牌背→牌面的渐变表达公开即可。
