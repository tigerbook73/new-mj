...
- **座位范围**：实时摸牌/弃牌的 ghost 与入场淡入适用于四个座位；God mode 只提供对手牌面，仍沿用正常的抓牌和弃牌动画。摸牌始终只填充固定最右摸牌槽，已有牌不会因其重挂载或重排。手牌 FLIP 仅用于 bottom 及未旋转的 top；left/right 位于四分之一旋转 Zone 下，通用 Motion FLIP 会沿错误屏幕轴移动，故这两方弃牌直接采用最终布局。
- **跨区域飞行**：摸牌/出牌/认领的适用动画统一走独立临时 ghost（`components/useFlightGhost.ts`：测量一次 rect → portal 到 `document.body` → 播完自毁），从不让真实业务节点（牌河墓碑、副露牌、待摸槽位）承担跨容器动画或被这层触碰。不用 `layoutId` 共享布局——墓碑永不卸载，隐式共享布局会把它当成正在退场，和自身的 `dimmed` 目标打架（具体机制见 `ClaimFlipGhost.tsx`）。对手弃牌的 `OpponentDiscardFlipGhost` 从整个手牌区起飞，不追踪某张特定手牌；对手暗手不携带可用于溯源的身份，铁律2同样适用。
