...
（下文"门清 claim 阈值"节）、碰碰胡弃牌 tiebreak（下文"碰碰胡结构路线"节）与清一色/混一色
弃牌方向（下文"清一色/混一色弃牌方向"节）是番型收益模型可行性专题的第一至第三个 slice，
均已加入生产（后两者均为 discard-only，不含 claim）；杠开和防守仍必须在后续独立 slice 中用
fixture/A-B 证据逐项加入，不能借此边界隐式改变。
...
清一色（×4）与混一色（×2）共享同一个核心结构信号——`packages/core/src/rulesets/junk/
scoring.ts` 的判定都是"非字牌部分只属于一种花色"，字牌有无只决定最终倍数（×4 还是 ×2），
不是两条独立构牌方向——所以合并为一个 slice 分析，不分别处理。

花色定向向听不需要新建 DP 表或碰 Layer 2 财神装饰层（`长期决策`§3；那层是为真正的癞子/
财神牌准备的，junk 本身无癞子，`docs/variants/junk.md`§1，也不是这里需要的机制）：
`evaluateUkeire`/`computeShanten` 从不要求输入正好 13/14 张，直接喂"手牌里只属于目标花色
+字牌"的过滤子集即可得到花色定向向听，复用现有算法。已验证方向正确：对一手接近清一色的
牌，弃掉杂色牌不改变过滤子集向听（该牌本来就不在过滤范围内），弃掉本花色牌则让向听变差，
跟直觉一致。三种目标花色（m/p/s）分别试、取向听最优。

接入条件比七对（任何副露都报废）、门清（任何非暗杠副露都报废）都更"软"：只要求已声明的
每个副露自身都落在目标花色（或全字牌）内，其余无限制——碰目标花色内的牌完全不影响花色
追求（甚至有帮助），只有碰了目标花色**之外**的牌才报废那个花色。判定见
`structural-discard.ts` 的 `meldsAllowFlush`。

第一版范围明确收窄：只做弃牌方向（onePly 排序 + `compareFinal` 最终 tiebreak），不碰
claim；只跟标准型路线比较，不与七对组合（清一色七对理论上可以叠乘到 ×8，但两条路线同时
追求的比较逻辑当时判定为本 slice 范围过大，留作后续评估）。后续已用诊断脚本核实：对
2000 个随机生成的 14 张暗手样本，分别算出标准型向听、七对向听（全手）、最优花色向听
（三花色取最优，同当前生产实现）和"花色过滤子集上的七对向听"（把 `sevenPairsShanten`
套在按目标花色+字牌过滤后的计数上，代表"花色七对"组合路线），统计后者是否曾经严格优于
现有三选一的最优值——结果 `0/2000`（0.00%），一次都没有。原因是七对向听公式
（`6-对数+max(0,7-牌种数)`）本质上依赖牌种多样性，把追求范围收窄到一个花色+字牌会把
可用牌种上限从 34 种砍到最多 16 种（9 数牌+7 字牌），这个限制几乎总是让七对变得更难
凑齐，抵消掉的收益远大于花色集中带来的好处；样本里"花色七对"路线相对现有最优的 gap
全部 ≤0（多数落在劣 1~3 级向听）。因此该组合方向已评估并放弃，不再是待办；不需要为
一个从未在采样分布中真正更优的路线支付额外的实现和验证成本。花色定向向听没有闭式公式
（不像七对的
`6-对数+max(0,7-牌种数)`），每次比较要对每个仍可行的花色各跑一次真实 shanten DP，比七对
的 O(1) 增量组合贵得多；为避免重蹈七对惩罚最初版本"每候选单独调未批量 `evaluateUkeire`"
的覆辙（当时一度让 P50 涨到 87%），花色信号被有意限制在只对**最多 5 个**已进入 shortlist
的候选调用（`evaluateStructuralDiscard` 的 `candidates` 映射与 `compareFinal`），不进入
2-ply continuation 每叶子调用一次的内层循环（该循环每次决策要跑上千次，花色版的每次比较
比七对惩罚贵得多，代价会更严重）。这个设计从一开始就避免了性能问题：实测单次弃牌决策 P50
`17.86ms` vs 不含花色信号的 `17.62ms`，几乎无差异，不需要像七对那样事后分三轮优化。

原计划仿照七对的分档惩罚（花色向听比标准型更难兑现，应该加惩罚），扫了 `handicap ∈
{0,1,2}` 做 200 seed 位置互换换位对局 A/B（`evaluateCandidatePolicies`），结果三个值
产生**完全相同**的决策（`evaluate policy diff` 0 差异）和换位对局数字（`191/400` 胜场
`47.8%`，净分 `+92`）。原因是花色信号只作用在"2-ply 已经打平的候选之间"这个很窄的
tiebreak 位置——不像七对渗透进整个 onePly+2-ply 主搜索，一个小整数惩罚在这个窄接入点上
翻不动任何比较结果。因此惩罚参数被直接移除（`bestFlushShapeOf` 用无惩罚的原始 min），
不保留一个已验证无效的常量。

`evaluate policy diff`（baseline = 门清阈值合入后的基线，candidate = 加入花色信号后）在
seed `20260814` 的 20 局种子、11825 个决策点中有 36 处（0.3%）动作不同，全部是弃牌——
比七对（3.5%）、门清（1.5%）都窄，符合"只在窄 tiebreak 生效"的预期。

本 slice 与上一节的碰碰胡 tiebreak 并行开发，两者的自对弈证据都是各自独立针对合入门清
阈值后的基线（即 v3）采集，不是彼此之上叠加的结果。落地顺序上碰碰胡先合并入生产并占用了
`structural-baseline@4`，本 slice rebase 到碰碰胡之上后把标识改记为 `@5`；两个 tiebreak
在 `compareFinal` 中各自独立生效、互不干扰（顺序为 continuation → flush → onePly →
pengpenghu），但 rebase 时未重新验证过二者组合场景的 A/B 效应，后续如需要应作为单独
candidate 评估。
