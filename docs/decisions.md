...
- **D16**（已被阶段 5 真实 Supabase OAuth 取代）：dev 假登录降级为无 `SUPABASE_URL` 时的开发态 fallback，机制见 `apps/server/src/gateway/auth.middleware.ts`。阶段 5.1 补充：配置了 Supabase 但 `auth.getUser` 校验失败时，仅在非生产环境（`ConfigService.isProduction` 为 false，即 `NODE_ENV !== "production"`）会重试这条 D16 校验作为兜底——根因是 D23 提交进 git 的 `.env` 带着 Supabase CLI demo 配置，导致本地 `pnpm dev` 的假昵称登录一律被判 unauthorized；生产环境必须设 `NODE_ENV=production`，该分支不触发，避免泄露的 dev secret 绕过真实鉴权。**本次修订（D28 同批次）**：非生产环境配置了 Supabase 时，改为先同步尝试 D16 校验、成功即放行，不再等一次可能连不上本地 GoTrue 的网络往返；只有不是有效 dev token 才落到原有的真实 Supabase 校验分支。生产环境仍完全跳过 D16 分支，安全性质不变。
...
- **D29【2026-07，P4.1 收尾】牌桌 Tile 尺寸计算用叶子容器 `ResizeObserver` + 纯函数 `fitTileGrid`，不用 CSS container query，也不用"顶层单点测量 + JS 全树推导百分比"**（判定标准，供未来类似跨端布局问题参考）：`cqw`/`cqh` 在 Expo/React Native 上没有等价能力，会挡住未来跨端复用；"顶层测一次、JS 逐层复刻百分比树"要求所有中间层元素零 margin/padding/border 才能保证算出的像素和实际渲染一致，是隐性且脆弱的不变量。改为只在真正需要"给定这块区域的实际像素，牌该多大"的叶子区域（`apps/web/src/lib/useMeasuredSize.ts`）用 `ResizeObserver` 测真实 content-box 像素（天然排除 border/padding），喂给零 DOM 依赖的 `apps/web/src/lib/tableGeometry.ts` 的 `fitTileGrid`；区域比例切分和旋转定位（`Ring`/`DirectionalSurface`，`components/mahjong/TableGeometry.tsx`）继续用普通 CSS 百分比 + `transform`，不受影响。开发态 Layout Lab（`/dev/table-layout`）与正式 Table 共用同一套测量与计算，不各自维护一份。
...
- **D31【2026-07，Table UX Phase 5】牌桌事件动画用 `motion`（`motion/react`，framer-motion 改名后的官方包），不用纯 CSS**：Phase 5a 起步时先用已装好的 `tw-animate-css` 零依赖起步，验证了"入场动画本身要多明显才看得见"（8px/200ms 太保守，肉眼几乎不可见，实测才调对参数）；用户要求评估切换后改用 `motion`，理由是后续 5c/5d 会需要"卸载前先播完退场动画"（`AnimatePresence`）——纯 CSS 只能手写 `setTimeout` 延迟卸载，脆弱且要为每个场景重新推导。踩过一个坑：`motion.div` 的 `animate` prop 一旦用上，会把目标值永久写成内联样式，内联样式天然盖过任何 CSS class/`:hover`——`Tile.tsx` 从 `<div>` 换成 `motion.div` 后，`hover:scale-*` 与 `dimmed` 的 `opacity-40` 都被静默失效过，从未被测试覆盖到，靠事后动图/截图排查才发现；修法是把这些状态也交给 motion 自己管（`whileHover`、把 `dimmed` 折进 `animate` 目标），不要在同一个属性上跟 motion 抢控制权。另一个经验（后续追加）：探索"认领的牌从牌河飞进副露"的跨区域 `layoutId` FLIP，机制上是成立的（实测 `transform` 在 ~300ms 内从明显偏移平滑衰减到 `none`，屏幕坐标真的跨过了棋盘），但共享 `layoutId` 会让 motion 把"这一帧这个 id 被另一个元素接管了"隐式当成一次 `AnimatePresence` 式退场处理——牌河那张牌会被自动淡出到 `opacity:0` 并加上 `pointer-events:none`，跟它必须永久保留、只是变暗的既定设计（架构铁律 4）冲突，且这个副作用不受 `layout="position"` 影响、压不住。**判定标准（供未来类似"共享位置但一方不真的消失"的场景参考）**：`layoutId` 共享布局只适合"旧元素真的会离开"的场景（配合 `AnimatePresence` 卸载）；旧元素要永久留着，就不能用 `layoutId`，改成一个独立的临时克隆（`ClaimFlipGhost.tsx`）——只在认领那一刻挂载，测一次牌河源位置和副露目标位置（`getBoundingClientRect`），用 `createPortal` 渲染一个 `position:fixed` 的克隆做纯位置/尺寸 FLIP，播完（`onAnimationComplete`）自己卸载；牌河墓碑和副露真实那张牌全程不接触这层机制，各自的入场/变暗逻辑完全不用因为"要不要参与 FLIP"而改动。判定 "activeIncremental" 快照是否该播动画的 scaffold（`useIsIncrementalSnapshot`+`usePrefersReducedMotion`→`canAnimateEntries`）目前只在 junk 桌面验证过，未跨玩法验证，暂不进 `architecture/key-designs.md`（见该文档收录标准）。
