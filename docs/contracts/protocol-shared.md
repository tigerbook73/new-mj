...
- `tabId`（客户端 `sessionStorage` 持久化，同 tab 刷新不变、新开 tab 换新值）/`browserId`（客户端 `localStorage` 持久化，同源全部 tab 共享）是账号级并发连接仲裁用的身份信号，握手**必带**，缺失/非字符串直接 `UNAUTHORIZED`；生成逻辑见 `apps/web/src/shared/lib/clientIdentity.ts`。
...
- **查询 = ack 给数据**：`lobby:list` 等，data 即答案，这次调用本身无后续事件（`lobby:changed` 是与具体某次查询无关的独立全局信号，客户端收到后自行决定要不要发起新一次查询，不是"这次 ack 之后还有事件"的例外，见 `session-mechanics.md` §6）
