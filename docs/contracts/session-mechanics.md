...
**房间（Room）是一次连续对局会话的容器**：4 个玩家坐进一个房间后会连打一串局（局数由会话配置决定），每局结束累加分数，直到会话结束、算出最终排名。房间只是"局的容器"，不实现任何玩法规则。
...
- **会话配置边界**：房间创建目前只允许 `totalGames`，可选值为 1、4、8，缺省为 4。它只决定固定局数会话何时结束，不进入 core `GameConfig`，不改变玩法规则、单局算分或 AI 推荐。`sessionFormat` 仍是服务端内部的既有会话类型标记，当前固定为 `"4-round"`；预留的 `"best-of-3"` 不开放给客户端，也尚未实现。
...
| `room:create` | 可指定房间名称和 `totalGames`（1、4、8；缺省 4）；`totalGames` 是 server-owned 会话设置，不进入 `GameConfig`。其他配置字段一律拒绝；"进入新上下文"类消息，ack 给快照                                                                                                     |
...
| `lobby:list`  | 查询，无副作用；`{ rulesetId, search? }` → `RoomSummary[]`，只返回 `phase==="waiting" && status==="open"` 的房间（一次性查询快照，search 按房间名称大小写不敏感子串匹配；MVP 仍不做观战；`lobby:changed` 信号触发客户端主动重新发这个查询，见下方"大厅列表变化推送"）    |
...
**声明窗口超时**：`ConfigService.claimTimeoutMs` 读取 `CLAIM_TIMEOUT_MS`，合法正整数原样采用（默认 `20000`，调试可设 `3600000` 等长值），缺失、空值、非有限数、小数、零或负数回退默认值。RoomService 不读取玩法私有 state，而是逐座位检查 `getLegalActions` 是否包含 `{type:"pass"}`；只有真人且未永久托管的合法 pass 座位才建立 timer。timer/deadline 属于 server 编排层，不进入 `Room`、core state 或 PlayerView。
...
**提前结束整场对局（`room:end`，2026-07 新增）**：`room:end` 与"托管"（`room:leave` 的 `in-game` 分支）是两件不同的事——托管只让发起者自己的座位转永久 AI，session 对其余座位继续；`room:end` 直接结束整场 session，对所有座位生效。任意已入座玩家都能发起，不复用 §6 局间确认的全员确认机制、无需其他人同意。实现上抽出 `RoomService.finishSession(room)` 私有方法，`shouldContinue` 为 false 时的自然结束路径与 `room:end` 共用同一段收尾逻辑：清算 `ranking`（用当前 `room.scores`）、置 `phase: "finished"`/`status: "closed"`、`untrackRoomPlayers`、写 `room.result`、`archiveSession`、广播 `room:sessionFinished`。调用前先清空该房间全部待执行 timer（claim/bot/draw-reveal）；若有一局正在进行且尚未打完，直接丢弃其内存态 `gameState`——不计分、不进 `finishedGames`，因此 `SessionResult.gamesPlayed` 统一改用 `room.finishedGames.length`（只统计真正打完的整局），不再用 `room.gameNumber`（后者在提前结束时会把打到一半的那一局也算进去）。

客户端有两个入口，都只发 `room:end`，不新增其他协议面：round-end 确认界面新增的"结束"按钮（不受 `isReady` 状态限制，已确认下一局的玩家也能改主意）；Leave room 确认框的"强制退出"选项（`room:end` ack 成功后紧跟一次 `room:leave`，此时命中 `finished` 阶段的 no-op 分支，只是让发起者自己的连接走既有的离开收尾）。同一确认框的"托管"选项完全不碰 `room:end`，只调用现有 `room:leave`。
...
| `lobby:changed`           | rulesetId                                | 见下方"大厅列表变化推送"                             |

**大厅列表变化推送（2026-08 新增，v2 改为信号制）**：三个会让某个 `rulesetId` 的大厅列表结果变化的时刻各广播一次 `lobby:changed { rulesetId }`——`RoomService.create()` 成功（新房间进入 `waiting`/`open`）、`RoomService.start()` 成功（房间离开 `waiting`，对 `lobby:list` 不再可见）、`leave()` 的房主在 `waiting` 阶段离开分支（房间被删，同一次连带广播的 `room:closed{reason:"hostLeft"}` 之外再加这一条）。

**只是一个信号，不带房间数据**：**全局广播**（`server.emit`，不按 `roomId` 或任何订阅频道过滤，不是新开一个"大厅频道"订阅机制）。客户端收到后，只在 `rulesetId` 与自己当前选中的玩法标签一致时，重新发一次已有的 `lobby:list` 查询（带上自己本地的 `search` 关键字）刷新列表；`rulesetId` 不匹配就直接丢弃，不查询。

**v1→v2 改动原因**：v1 版本（`lobby:roomCreated`，只推新增、payload 带完整 `RoomSummary`）会导致大厅列表只增不减——房间开局或房主关房后，其他人本地列表里残留一条点了就报错的失效房间。v2 统一用"信号 + 客户端重新查询"覆盖增、减两个方向：好处是服务端不用在三个触发点各自重算并广播一份房间列表（`search` 又是纯客户端本地状态，服务端广播时也不知道该按谁的 `search` 过滤），直接复用已有、已测试过的 `lobby:list` 查询/过滤逻辑，用一次额外的 ack 往返换掉"服务端到处维护列表投影 + 客户端做增量合并/去重"这两块重复实现；大厅列表刷新本来就不是延迟敏感的路径，多一次 round trip 可以忽略。

**已知未覆盖**：座位加入/离开导致的人数变化（`playerCount`）不会触发这个信号——房间满座（4/4）前仍是 `waiting`/`open`，本来就该继续出现在大厅列表里（`lobby:list` 本身就会返回它，`room:join`/`room:addBot` 撞满座时报 `SEAT_TAKEN`/`ROOM_FULL` 已经是现成的错误路径），只是列表上显示的人数在下次刷新前可能不是最新的，不算"房间消失"这个范围要解决的问题。
...
- ✅ `lobby:changed`（2026-08 新增）：新房间创建、开局、房主关房三个时刻广播信号（不带房间数据），客户端按需重新查询 `lobby:list`，见 §6"大厅列表变化推送"；座位加入/离开导致的人数变化不触发，见下
...
- ❌ 大厅列表因座位加入/离开（人数变化，非开局/关房）而刷新（`lobby:changed` 只覆盖新增/开局/关房三个时刻，见 §6"已知未覆盖"）
...
2. **同一个浏览器、不同 tab**（`browserId` 相同、`tabId` 不同）、且旧连接仍在 registry 里（即仍活跃）→ 无条件拒绝，握手以新错误码 `SESSION_EXISTS_SAME_BROWSER` 失败，`takeover:true` 在这条分支不起作用（没有接管入口）。客户端（无论是走 `ensureConnected()` 被动恢复还是 `LoginView`/`AuthCallbackView` 显式登录撞上这个码，见 §12）一律跳转独立的 `/session-blocked` 死路页（`apps/web/src/features/auth/SessionBlockedView.tsx`）：只尝试 `window.close()`（对非脚本打开的普通 tab 是 best-effort，大概率静默失败），**不清任何本地凭证**——`localStorage` 里的 dev-session token / Supabase session 是同浏览器全部 tab 共享的，旧连接那个 tab 还在用同一份凭证，清掉会把它一起挤下线；即使这个死路页所在的 tab 被刷新，`tabId` 仍是同一个值（`sessionStorage` 语义），会再次确定性地撞回同一个错误码，不存在"死页面里刷新反而抢到会话"的风险。
...
**提前结束整场对局**（2026-07 新增，见 §6"提前结束整场对局"）

```
# in-game 阶段，任意已入座玩家发起，无需其他人确认
Player D: room:end {}
  ← ack: { ok: true, data: {} }
  [第 3 局正打到一半：直接丢弃该局内存态，不计分、不进 finishedGames]

房间内所有人: room:sessionFinished { result: { winner, ranking, format, gamesPlayed: 2 } }
  [gamesPlayed 只统计已经打完的前两局]

# 若这是 Leave room 对话框里的"强制退出"选项，客户端紧接着再发一次
Player D: room:leave {}
  ← ack: { ok: true, data: {} }
  [phase 已是 finished，命中 no-op 分支——只是让 D 自己的连接走正常离开流程]
```
...
- **明牌 replay**（`debug:replayOmniscientView`）是调试/测试专用逃生舱，只支持局终（直接读 `finalState` 喂 `getOmniscientView`，不做事件重放），鉴权模型完全不同于 `replay:get`（要求当前是房间已入座玩家，跟直播版全明牌一致），不进正式产品 UI——完整说明见 `contracts/protocol-shared.md` §7。
...
- **鉴权**：`auth.middleware.ts` 按 `ConfigService.supabaseUrl`/`supabaseServiceKey` 是否配置分支——未配置就走开发态共享 HS256 校验（dev-JWT fallback）。配置了且 `ConfigService.isProduction` 为 false（`NODE_ENV !== "production"`）时，**先同步尝试同一份 dev-JWT HS256 校验**（不等一次可能连不上本地 GoTrue 的网络往返）；不是有效 dev token 才用 `@supabase/supabase-js` 的 `auth.getUser(token)` 委托 Supabase 自己的服务器验证真实 token（`socket.data.userId` = Supabase 返回的 `user.id`，同时 fire-and-forget upsert 一行 `profiles`）；两者都失败才 `UNAUTHORIZED`。生产环境（`NODE_ENV=production`）完全跳过 dev-JWT 分支，只走真实 Supabase 校验——这是为了让提交进 git 的 `.env`（Supabase CLI demo 配置）不会挡住本地 `pnpm dev` 的假昵称登录，同时保证泄露的 dev secret 在生产环境完全没有可乘之机。两条主路径仍共用同一个中间件函数，靠配置存在与否分支，不是新增一个"测试模式"开关，现有全部 e2e（不设 `SUPABASE_URL`）零改动继续用旧路径。

## 12. 客户端会话恢复：server-truth 优先
...
- **client 端不再有单独的"恢复"状态机**：`apps/web/src/shared/lib/sessionBootstrap.ts` 的 `ensureConnected()`/`doConnect()`/`establishSession()` 只负责"连上 socket + 拿身份 + 挂 `session:kicked`/`disconnect` 监听"，不做任何路由判断；`session:kicked`/`disconnect` 触发时只重置 store 状态（`socket:null` 等），不 `navigate()`、不整页刷新。
...
- 服务器重启后，内存态房间本身就不存在了（架构未变，见 §11 开头），`playerRooms` 和"client 记住的 last-room"两种机制在这种情况下都救不回来——这是 server-truth 相对 client-only 机制严格更优、而非"各有取舍"的原因。
