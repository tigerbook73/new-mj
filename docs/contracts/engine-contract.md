...
- `applyAction(state, seat, action)`：非法即返回 `RuleViolation`（含机器可读 code），state 不变；server 超时代提交的 `pass` 与玩家主动 `pass` 完全同型
...
- **事件重建 ≡ 直接派生**：任意时刻，事件流重建的视图必须等于直接调用 `getPlayerView` 的结果。测试参数化遍历已注册的 ruleset（`packages/core/test/cross-ruleset-invariants.test.ts`），新增玩法只需要把自己的 ruleset 加进注册表，不必重写测试——测试策略见 `testing-strategy.md`。
