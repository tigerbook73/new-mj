...
只计算弃牌后的即时番型，没有计算“弃牌后走向清一色”的轨道潜力。结论是：不能只把
现有番型权重调大，下一步需要单独设计低成本的番型/花色轨道预评分。
...
- 低成本实现候选已记录但暂不深入：每个弃牌候选只用 O(34) 牌面计数计算主花色牌数、
  杂色牌数、字牌数、对子/刻子保留，再形成清一色/混一色轨道潜力分；复用已有计数，
  不新增 core API、不重复运行 shanten。恢复实验时先验证能否保留 `pure-suit-drift`
  路线，再决定是否进入动态 N。
