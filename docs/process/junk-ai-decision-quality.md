...
| 优先级 | 方案 | 当前证据 | 结论与准入条件 |
| --- | --- | --- | --- |
| A | 首轮加权上断崖 + 第二轮低分端下断崖（20%） | 1000 case：约 21.79ms/case、100% 一致、平均分差约 0；全量约 97.94ms/case，约 4.49x 加速；平均首轮 4、第二轮 7.875 个候选 | 当前最有潜力。可进入 AI 自对弈 A/B；仍不接入默认路径，高番型 stress 和真实对局需验证。 |
| A | 在上述方案中使用 core shanten-only batch | 已包含在 21.79ms/case 测量中；相对全量 2-ply 约 4.49x | 作为 A 方案的实现优化保留；不改变评分语义，不单独改变默认策略。 |
...
两轮使用同一套“相邻分数曲线”思想，但方向和目标不同，不能共用同一个 N 规则：

```ts
type DynamicDiscardConfig = Readonly<{
  upper: Readonly<{
    minN: number;          // 首轮最低候选数，当前建议 2
    maxN: number;          // 首轮最高候选数，当前建议 4
    relativeGap: number;   // 从高分端遇到断崖的相对阈值
  }>;
  lower: Readonly<{
    minN: number;          // 第二轮最低保留数，可按质量实验调整
    maxN: number | "all";  // 第二轮最高保留数，默认 all
    relativeGap: number;   // 从低分端遇到断崖的相对阈值
  }>;
}>;
```

上断崖用于首轮性能控制：按“弃牌价值”从高到低排列，至少评估 `minN`，从高分端
开始扫描；遇到第一个超过 `upper.relativeGap` 的相邻落差就停止，最多不超过
`maxN`。当前建议 `minN=2,maxN=4`，因为 Top-1 单独实测只有 87.5% 一致率。

下断崖用于第二轮质量保护：仍按“弃牌价值”排序，但从低分端开始扫描；低分区域与
后续候选之间出现第一个超过 `lower.relativeGap` 的落差后，保留断崖后面的全部候选。
如果没有断崖，则保留全部候选。第二轮不能声称数学精确，只能说在无断崖时退化为全量，
比固定 Top-4 更保守。

两个阈值应独立配置，因为首轮目标是省计算，第二轮目标是避免漏掉最佳弃牌；两轮的
断崖位置也不要求相同。比例建议使用归一化相邻下降，而不是简单的最大值/最小值比值，
以避免负分、零分和整体权重缩放造成不稳定。排序本身最多处理 14 个牌种，成本可忽略；
真正需要测量的是为了得到第二轮曲线而新增的评分计算。

最终选择列表：

1. **首选实验方案**：首轮加权上断崖（`minN=2,maxN=4,relativeGap=20%`）+ 第二轮低分端
   下断崖（`minN=1,maxN=all,relativeGap=20%`）+ core shanten-only batch；1000 case
   实测约 21.79ms/case、约 4.49x 加速、100% 一致，平均分差约 0。
2. **质量优先备选**：在首选方案前增加最低向听层黑名单；只绑定当前 `JunkWeights`，
   权重变化必须重新 A/B，不能作为无条件安全剪枝。
3. **候选生成备选**：加权/轨道预评分 Top-4；高番型 stress 仍有漂移，只作为候选
   生成器，不单独进入默认路径。
4. **不再选择**：固定 Top-N、结构 Top-4、full-result batch、跨候选共享缓存、
   候选×摸牌统一批处理和 Top-2 分差回退。
...
| 第二轮动态断崖白名单             | 接受为诊断候选、暂不默认。平坦时扩大候选、出现断崖时收缩；旧绝对断崖 1000 case 平均 7.824 个候选、约 24.33ms/case；改为上下独立的归一化相对断崖 20% 后，首轮平均 4、第二轮平均 7.875 个候选，约 21.79ms/case，100% 一致，平均分差约 0（约 9.5e-9 数值误差）。 |
...
本轮进一步测试了双向相对断崖：相邻分数下降除以该手牌候选曲线的完整分数区间；首轮从高分端扫描，至少保留 2 个、最多 4 个，第二轮从低分端扫描，遇到 20% 断崖后保留断崖上方全部候选，无断崖则全量。1000 个确定性 case 中方案与全量 2-ply 胜者一致率 100%，平均分差仅约 `9.5e-9` 的浮点误差。实现仍是诊断路径，第二轮复用首轮排序作为近似白名单，不能声称数学安全。
