...
| --- | ---: | ---: | ---: |
| 1-ply | 78.125% | 0.4541 | — |
| 2-ply Top-1 | 78.125% | 0.4541 | 8.53 |
| 2-ply Top-2 | 90.625% | 0.1038 | 16.70 |
| 2-ply Top-3 | 93.750% | 0.0697 | 24.95 |
| 全量 2-ply | 100% | 0 | 94.00 |

Top-1 与 1-ply 一致是预期的，因为它们选择的是同一个 1-ply 第一候选。性能收益明确，
但决策漂移也明确，当前结论为**拒绝直接接入默认评分**，转入分级 2-ply 评估。
...
| Top-N 2-ply 直接替代全量候选          | 拒绝。32 手牌/96 case 中 N=1/2/3 一致率仅 78.1%/90.6%/93.8%；虽约快 11.1x/5.6x/3.8x，但决策漂移不可直接接受。 |
| Top-2 分差触发全量                  | 拒绝。阈值 0–2 仍有 3.125% 漂移；阈值 5 才全一致但 90.625% case 回退，估算比全量更慢。 |
| 1-ply 分数作为 2-ply 安全上界          | 拒绝。Top-1 相对全量 2-ply 仅 78.1% 一致；现有 1-ply 只是排序启发式，不是可证明上界。 |
