...
- 动态 Top-N 诊断已实现：默认 `minN=2`、`maxN=8`、分数窗口 4、断崖阈值 12；1000 个
  固定随机 case 平均保留 6.875 个候选，一致率 100%、平均分数遗憾 0、58.939ms/case；
  12 个默认结构 fixture 也全部一致。高番型 stress 仍有个别漂移，不能直接接入默认策略。
