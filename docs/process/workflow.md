...
- 私有 workspace 包 `@new-mj/devtools` 是 worktree CLI、slot 推导和 Vite/Playwright 适配的唯一来源；它以 `new-mj-worktree` bin 提供 CLI，根脚本只调用该稳定命令。使用其库 API 的 app 以 `workspace:*` devDependency 引入。`.worktree.env` 只记录本机 slot，端口由 slot 推导，具体 slot/私有环境不进入 Git。
