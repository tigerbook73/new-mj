...
> 状态：Phase 3 已本地 squash merge；Phase 4 改为独立 branch 的逐场景检查点，P4.1 桌面横屏（含手牌排序、Storybook、Layout Lab、正式 Table 接入）已用户验收并本地 squash merge，P4.2 手机横屏待制定详细计划。
>
...
### [~] Phase 4 — 视觉基础与响应式 Table 骨架（P4.1 已合并，P4.2 待制定详细计划）

Phase 4 子阶段分支与 merge 记录：

| 子阶段            | Branch                         | Merge commit |
| ----------------- | ------------------------------ | ------------ |
| P4.1 桌面横屏     | `feat/table-layout-desktop`    | `cda9286`    |
...
- 浏览器人工验收（真实四人开局 + 1440×900/1366×768 视口目视检查）已于 2026-07-20 完成并通过，P4.1 整体验收通过，本地 squash merge 为 `cda9286`（见上方子阶段表）。
