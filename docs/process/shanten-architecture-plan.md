...
- **Layer 0 实现结果（已完成）**：算法设计（把递归拆成"单花色子表 + 4 花色
  ...
  - **后续追加两个剪枝（已实施、已实测）**：①总张数 >14 的向量不建表——
    任何分支都是从 counts 里减牌再递归，子状态总张数只会更少不会更多，超过
    ...
    字牌表从约 20ms 降到约 15ms。端到端自对弈耗时（同一段驱动脚本）从上一轮
    的 112ms/局 再降到 **约 82.5ms/局**，相对最初基线（982ms/局）总提升约
    **11.9x**。
  - **两级存储（已实施、已实测）**：`SuitTable` 从"稠密数组直接
    `data[vectorIndex*15]` 寻址"改成"`indexMap: Int32Array` 做
    `vectorIndex→紧凑下标` + 紧凑 `data` 只存有效结果"——起因是 >14 剪枝加
    镜像去重后，稠密数组里 89.6% 都是从未写过的哨兵值。数牌表两级布局体积
    从 27.9MB 降到约 **10.9MB（约 2.7x）**：`indexMap` 约 7.5MB（全量
    195 万个下标，Int32），紧凑 `data` 约 2.9MB（只存 203,122 个去重后的
    有效向量）。查表吞吐量交替对照测量（不交替会有 JIT 预热/顺序偏差，第
    一次误判成"快 22%"，交替测量后修正）下跟稠密布局基本打平（约
    1.01x）——紧凑数组体积小更容易被 CPU 缓存命中，抵消了多一次间接寻址
    的理论开销，内存收益基本没有速度代价。生产代码接入后端到端自对弈耗时
    仍是约 83~85ms/局，跟接入前一致（这轮改动是省内存，不是提速）。多线程
    建表仍留作后续，目前数字已经足够好，没有必要再加这层复杂度。
  - **内容级 hash-cons（与两级存储同一 commit 实施，此前漏记）**：`data`
    记录内容重复率极高（数牌 203,122 条只有 1,119 种、字牌 21,735 条只有
    47 种），按内容去重后 `data` 压到约 17KB，`indexMap` 随 contentId 上限
    骤降收窄成 Int16Array；数牌表总大小从约 10.9MB 降到约 **3.74MB**。
  - **查询热路径常数优化 + 镜像折叠寻址（已实施、已实测，交替对照）**：
    ①查询侧 `blocks`/counts 改模块级复用、合并 DP 的 melds 上限从宽松的
    16 收紧到真实上界 4，单次查询从约 1.40µs 降到约 0.84µs；②把 rank 反转
    镜像对称直接编码进 `indexMap` 寻址（x/m/y 拆位 + 三角数编址，镜像对
    共享槽位），`indexMap` 减半（数牌约 3.72MB→1.87MB、字牌 153KB→77KB，
    表总大小约 1.93MB），叠加后单次查询约 0.79µs（合计约 **1.78x**）；
    ③建表 `data` 稠密预分配（约 29MB 瞬时）改倍增扩容，建表耗时持平。
    寻址正确性由两条新穷举安全网（槽位单射性、镜像内容一致性，双表全量）
    加既有差分/对拍用例兜底。
