...
### 番型路线收益模型可行性：剩余番型（清一色/混一色/碰碰胡/杠开）
...
剩余四个番型仍是候选，不假设能直接照搬门清的做法：清一色/混一色/碰碰胡需要新的花色/刻子
追踪维度（`StructuralShape` 目前不含这类信息），成本明显高于门清那种"复用已有 existingMelds
式布尔信号"的情况；杠开只在自摸且连杠链不断时生效，属于时机/连续动作问题而非构牌方向问题，
可能根本不适合同一套"claim 阈值"框架。开始前先评估现有 `structural-routes.ts` 的
