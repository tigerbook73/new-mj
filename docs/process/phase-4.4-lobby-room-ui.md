...
| **4.4.3 标签页大厅入口**    | `/games` 从"两个按钮"改成 Tabs（junk / bloodbattle），下方联动显示对应大厅内容（与 4.4.5 合并实现）                                                                                  | e2e：切换标签内容跟着变                                                     | ✅   |
...
| **4.4.6 离开房间**          | web：房间页和 table 页提供"Leave room"；等待阶段调用 `room:leave`（房主离开会被踢回大厅并看到房间已关闭提示，非房主离开正常回大厅）；对局中同样提供离开按钮，效果同断线              | e2e：非房主离开、房主离开、对局中离开三条路径                               | ✅   |
