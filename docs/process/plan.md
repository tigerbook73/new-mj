...
阶段 4 系列的前三个子阶段（4.1 AI 补位、4.2 断线托管、4.3 黑暗模式）已完成，下一个子阶段是 4.4（UI/操作优化，用户逐项描述中），详细方案见 `phase-4-junk-complete.md`（阶段 4 系列收尾后按 `../doc-map.md` §6 吸纳耐久内容并删除）。

**下一步第一个动作**：等用户列完 4.4 的具体条目再排期实现。

> 4.3/4.4 范围调整：原计划里的"界面/操作优化"一开始没有具体条目（用户原话只圈定了"纯 UI、不涉及后端"这个边界），先决定拆出去推迟到有具体反馈再排期；随后用户改主意，改为单独立项为 4.4，由用户逐项描述具体内容，原 4.4 Replay / 4.5 持久化各自顺延为 4.5 / 4.6。4.4 的具体条目见收尾时补充到本文件（收集过程中的往来不留痕，只记定案）。
>
