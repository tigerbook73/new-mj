...
步骤 0、0b、1、2、3、4、5、6、7 已完成；下一步进入步骤 8：`isolationPotential` 影响边界校准。生产策略保持 weights、无权重 analysis/cache、hand-quality、2-ply continuation、action-scoring 和兼容 facade 的单向边界。
...
下一步第一个具体动作：开始步骤 8 前先补充 `isolationPotential` 专门计划，固定只在向听、存活进张种类/张数和结构 2-ply 均不能区分的候选上测量其边际影响，建立启用/禁用的 paired fixture 与报告验收；先做只读比较，不修改默认权重或生产选择。
...
- 8 下一步：isolationPotential 影响边界校准
