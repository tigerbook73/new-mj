...
- 跑一次真正规模的 junk AI 调参（`pnpm tune:junk`，默认参数已经是"真跑"量级：`--max-generations 100 --min-generations 20 --seeds-per-generation 6`，会自己判断收敛提前停，不用手动猜代数；性能上比最初快约 7 倍，Shanten Layer 0 预计算表落地后自对弈单局又再降约 8.75x，两轮优化叠加），人工 review 报告后决定是否 `--write` 采纳候选权重（或手动改 `default-weights.json`）。
