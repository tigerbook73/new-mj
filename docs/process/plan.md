...
1. [ ] 盘点现有 AI/Junk 测试并重分类：区分结构契约、策略回归、2-ply 正确性、arena/worker、policy loader 和调参平台；基础牌形期望迁移到 `structural-calibration.test.ts`，依赖当前权重的具体选牌保留为 policy regression。先迁移和标记，不直接删除；只有新诊断覆盖后才删除重复用例。
