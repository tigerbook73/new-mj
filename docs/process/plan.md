...
- 低成本轨道预评分方案已记录、暂不实现：对每个候选仅做 O(34) 牌面计数，计算主花色、杂色、字牌和对子/刻子保留，再按权重形成清一色/混一色潜力分；复用已有计数，不新增 core API、不重复运行 shanten。
...
下一步第一个具体动作：暂缓低成本番型/花色轨道预评分实现；恢复该实验时先验证 `pure-suit-drift` 路线，再用 baseline 比较动态 `minN`/`maxN`、分数阈值和断崖规则；core batch API 继续等待架构确认。
