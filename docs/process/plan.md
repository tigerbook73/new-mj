...
- weighted tuning/evaluation 专用闭包已删除：weights tune/compare CLI、optimizer、tune worker/任务、isolation boundary/removal、paired isolation validate、旧 weighted-vs-structural compare/trace 壳及其专用测试和 backlog 均不再保留。跨模块换位对局迁为中性 `policy-match.ts`，与 `policy diff`、capture、arena、scenario provider、structural teacher 和通用 worker pool 继续服务 structural baseline/candidate；CLI 不再提供权重写回或以 legacy 为默认参照的命令。
- weighted runtime 已从当前树删除：公共 `recommendJunkAction`/`chooseJunkAction` 只接受真实的 view/actions 参数并固定调用 `structural-baseline@1`；legacy facade、默认权重、旧 analysis/hand-quality/action-scoring/two-ply/概率闭包及其内部测试均已删除。policy loader/capture/arena/match 只处理 baseline/candidate module/export，不加载权重；weighted/one-ply/two-ply evaluator 与六份历史 baseline 资产也已移除。旧实现仍可从 Git 历史回溯，但不再是当前生产或评估依赖。

## 下一步第一个具体动作

建立 `docs-and-names` slice：从 package public exports 和 Junk 源码根开始，沿 production、evaluation CLI、fixture、测试、README、架构、backlog 和 plan 做 top-down 依赖与叙述审计；删除残留的 legacy/weighted 默认命名、失效链接和不再存在的命令/资产说明，但保留明确标注为历史设计结论的内容。用 `rg`、文档链接检查、AI `verify:full` 和根 `pnpm verify` 证明当前树只描述 structural production 与通用 baseline/candidate 演进路径。
