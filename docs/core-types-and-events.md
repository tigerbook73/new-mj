...
**安全纪律**：id→kind 映射静态公开，因此 TileId 与牌面同级敏感。

- 不得在 public 事件中暴露仍处于隐藏状态的牌的 id（如手牌、暗杠）
- 已公开的牌（出牌、副露中的牌、胡牌快照）可在 public 事件中包含 id，支持 UI 精准动画
- 具体到事件：`TileDrawn` 的 public 版本不携带 id；`TileDiscarded`、`PengMade`、`GangMade` 等 public 事件携带 id（因为都是已公开的牌）

**测试约定**：fixture 用例以 kind 书写（如 `[1m,2m,3m]`），由加载器自动派发 id。

## 2. 状态形状（按 ruleset 私有）

- 每个 ruleset 在 `rulesets/<id>/types.ts` 定义自己的 `<Id>State`（`JunkState`、`BloodbattleState`……），没有跨玩法共享的全局 `GameState`（D12）
- 公共子结构 `SeatState`/`Meld`/`DiscardEntry`（见 §1）来自 `lib/seat.ts`
- `JunkState`/`JunkPendingClaims` 见 `packages/core/src/rulesets/junk/types.ts`；`BloodbattleState` 见 `packages/core/src/rulesets/bloodbattle/types.ts`
- `BloodbattleState` 顶层的 `exchange?`/`lack?` 字段存放换三张/定缺的私有状态；不存在单独的 `variantState` 命名空间（取舍理由见 decisions.md D12）

engine-api 的公共骨架（`packages/core/src/types.ts`）只保留跨玩法共用的形状：
...
- **容器唯一性约定**：任一 TileId 任意时刻物理上只归属一个容器——牌墙 / 某家手牌 / 某家牌河活跃条目（`claimedBy` 为空）/ 某家副露 / 胡牌快照
  - 牌打出即入出牌者牌河；被吃/碰/杠时，牌的物理归属移入声明者副露（`Meld.from` 记录来源家），牌河条目原位保留并置 `claimedBy`（墓碑，不计入守恒）
  - "牌集守恒"不变量 = 上述容器的 TileId 并集恒等于完整牌集且两两不相交；附加不变量：每个墓碑条目的 TileId 必出现在 `claimedBy` 家的某个副露中
- `pendingClaims.options` 只包含至少有一个合法响应的座位；无选项的家不进窗口、无需表态，server 的超时计时也只针对这些座位（取舍理由见 decisions.md 评审点 B；已知泄漏见评审点 E）

### 2.1 跨局：庄家轮换

`createGame` 的庄家不是 core 内部自选的——`dealer: SeatId` 是调用方必须提供的显式参数（谁开局摸 14 张、谁先手）。谁来决定这个值分两种情况：

- **第 1 局**：房主的座位，纯房间/座位安排，与麻将规则无关——server 直接决定，不经过 core。
- **第 2 局起**：core 用一个跟 `createGame`/`applyAction` 同级的 dispatch 方法给出：`computeNextDealer(finishedState, currentDealer) → SeatId`。`finishedState` 就是刚打完那局的最终状态——不是一个新发明的类型，是调用方已经手上握着的同一个不透明 `TState`。

两个 ruleset 今天都是"不看结果，顺时针轮转"——`junk` 规则规格明确"不记连庄"，血战定稿规格未提连庄。但这是一个真实存在、按 ruleset 可能分叉的规则点（不是投机接口）：函数体今天忽略 `finishedState`，但签名已经把"上一局怎么打完的"喂给了公式，未来要支持连庄/抢庄/叠倍之类的变体（不同规则对"连庄要不要翻倍""能不能被抢"的答案并不相同），只改对应 ruleset 里这一个函数，不动 server 编排逻辑（decisions.md D15）。目前没有跨局记忆的真实需求（比如"连庄 N 次强制轮庄"要数连续赢局数），所以没有引入额外的持久化会话状态——真有这个需求时再加，避免未用先建（D12 的教训：engine-api 曾因预建了没人用的接口而收缩过一次）。
...
`engine-api` 的 `applyAction` 签名对外只承诺"入参是某个已注册 ruleset 私有的 Action 类型"，具体判别联合由各玩法在自己的 `types.ts` 定义；不存在跨玩法共享的全局 `Action`（D12）。

- `JunkAction` 见 `packages/core/src/rulesets/junk/types.ts`（discard/anGang/buGang/zimo/chi/peng/minGang/hu/pass）
- `BloodbattleAction` 见 `packages/core/src/rulesets/bloodbattle/types.ts`（前置阶段的 exchangeThree/chooseLack，playing 阶段动作与 junk 同构复用）
...
信封结构（`GameEvent`/`EventVisibility`/`EVENT_TYPES` 常量）见 `packages/core/src/events.ts`。下表是权威汇总视图，不是代码重复：各事件的具体 payload 是内联对象字面量，散落在各 ruleset 的 `state-machine.ts`/`settlement.ts` 里，没有集中的具名类型可指。
...
`GameStarted`（#1）的 `dealer` 字段语义不变——依然是"谁这局摸 14 张"，只是来源从 core 内部 PRNG 抽取改成了调用方传入的显式参数（见 §2.1）。

非法动作不进事件日志（不改变状态），仅作为 `applyAction` 的错误返回：server 以 ack 拒绝该客户端，并记入 server 侧错误日志（logger，非游戏事件；取舍理由见 decisions.md 评审点 D）。

`ClaimWindowOpened` 按座位拆分发送各自的选项，避免泄漏他家能做什么。但窗口的**存在与时长**是全桌可观察的行为特征：

- 出牌后瞬时推进 = 无人有响应权
- 出现停顿 = 至少一家能碰/杠/胡（即使其最终 pass，"能碰而未碰"已是有价值的读牌信息）——已知泄漏，按 D7 口径不做混淆处理（取舍理由见 decisions.md 评审点 E）
...
engine-api 的公共骨架 `PlayerViewBase`（`packages/core/src/types.ts`）只含跨玩法一致的字段：`seat`/`hand`/`seats[].handCount`/`wallCount`/`currentSeat`。

玩法私有字段不放进公共骨架，各 ruleset 用交叉类型扩展：

- `phase`/`myClaimOptions`/`myClaimResponse`/`lastDiscard`/`result` 以及公开副露、牌河的具体表示都是玩法私有的
- junk 使用 `TileId` 形式的 `melds`/`discards`，bloodbattle 使用对外安全的 `TileKind` 形式——避免不同玩法的牌面敏感性和数据形状混在一起
- `JunkPlayerView` 见 `packages/core/src/rulesets/junk/types.ts`

血战 `PlayerView` 每个公开 seat 额外携带：
