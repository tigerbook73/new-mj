...
- `src/gateway/`：`RoomsGateway`（消息路由 + EventBus 订阅；`reply()`/`replyAsync()` 两个 helper，后者只给 `replay:get`/`debug:replayOmniscientView` 这两个可能碰 DB 的 handler 用；`session:identity` 的 ack 带 `activeRoom`，来自 `RoomService.findActiveRoomForUser()`，见 `protocol-shared.md`）、`auth.middleware.ts`（握手鉴权，阶段 5 起按 `supabaseUrl`/`supabaseServiceKey` 是否配置分支真实 Supabase 校验 vs D16 开发态校验；非生产环境配置了 Supabase 时优先尝试 D16 开发态 JWT（同步快速路径，不等一个可能没起的本地 Supabase），不是有效 dev token 才走真实 Supabase 校验，两者都失败才 UNAUTHORIZED，生产环境跳过 D16 分支，见 `decisions.md` D16）、`connection-registry.ts`（座位↔socket 映射）、`gateway.module.ts`。
