...
- `src/gateway/`：`RoomsGateway`（消息路由 + EventBus 订阅；`reply()`/`replyAsync()` 两个 helper，后者只给 `replay:get`/`debug:replayOmniscientView` 这两个可能碰 DB 的 handler 用）、`auth.middleware.ts`（握手鉴权，阶段 5 起按 `supabaseUrl`/`supabaseServiceKey` 是否配置分支真实 Supabase 校验 vs D16 开发态校验；Supabase 校验失败且非生产环境时会重试 D16 校验兜底，见 `decisions.md` D16）、`connection-registry.ts`（座位↔socket 映射）、`gateway.module.ts`。
