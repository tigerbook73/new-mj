...
const middleware = createAuthMiddleware(jwtService, configService);

const fakeSocket = (auth: Record<string, unknown>): Socket =>
  ({ handshake: { auth }, data: {} }) as unknown as Socket;

describe("createAuthMiddleware", () => {
  it("rejects a protocolVersion mismatch with VERSION_MISMATCH", () => {
  ...
  it("rejects a missing/invalid token with UNAUTHORIZED", () => {
  ...
  it("accepts a valid token and binds socket.data.userId from its `sub` claim, ignoring any payload userId", () => {
