...
- `src/views/`：`LoginView`/`GamePickerView`/`LobbyView`/`TableView`。`GamePickerView` 负责玩法 Tabs、`lobby:list`、搜索和建房；`LobbyView` 使用 `/lobby/:roomId` 的 `room:peek`，支持指定座位入座/加 bot/离开；`TableView` 只渲染 `PlayerViewBase` 公共骨架，同时提供复用 `room:leave` 的离开入口，不按 `rulesetId` 分支，血战定缺/换三张这类专属阶段没有对应 UI，卡在那个阶段发不出动作是预期行为。
