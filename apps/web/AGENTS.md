...
- `src/store/session.ts`：`useSessionStore`（socket 实例、用户、房间、`PlayerView`、单局 `gameSeq`、`kicked` 标记、`activeRoomHint` 这个 `session:identity` 给的轻量恢复线索——一旦 `setRoom()` 真正拿到完整 `RoomInfo` 就会清掉，避免过期线索把人带回已经离开的房间）；`applyGameSnapshot` 是 PlayerView 唯一写入口，房间成员/准备状态继续用对应 room event action 增量更新。
