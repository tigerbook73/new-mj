...
7. **分层**（依赖规则强制）：core 基建不含玩法逻辑；玩法只进 RuleSet 或 config（D8 边界）；server/client 不实现规则（UI 由 myClaimOptions/getLegalActions 驱动）。
...
- `packages/core/src/engine.ts`：对外 `applyAction` / RuleSet 路由。
- `packages/core/src/rules/junk.ts`：junk 流程、结算、PlayerView 派生。
- `packages/core/src/{tiles,prng,wall,win,invariants}.ts`：纯基础设施与不变量。
- `packages/core/src/{events,view-reducer}.ts`：可见性事件与客户端视图重建。
- `packages/core/src/{simulate,cli}.ts`：可重放整局模拟与 CLI/fuzz 入口。
